	...
	async execute(interaction) {
        ...
        collector.on('collect', async i => {
            ...
            if (i.customId === 'confirm'){ 
                ...
                const mentions = selectedUserIds.map(id => `<@${id}>`).join(', '); // map ID to a user's @name
                ...
                * Prompt the user for the reason for the submission */
                ...
                const modal = new ModalBuilder()
                    .setCustomId('deduction_reason')
                    .setTitle('Reason for deduction');

                const reasonInput = new TextInputBuilder() // creating texet input field
                    .setCustomId('reason')
                    .setLabel(`Why are you deducting points?`)
                ...
                try {
					...
					const modalInteraction = await i.awaitModalSubmit({
						filter: (modal) => modal.customId === 'deduction_reason' && modal.user.id === interaction.user.id,
						time: 3_600_000,
					});
					...
					await modalInteraction.reply({
						content: `${initiatorNickname} deducted points from ${mentions}.\n\n**Reason:** ${reason}`,
					});
